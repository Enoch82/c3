...
function run(id) {
  active += 1;
  const out = join(root, "renders", `slide-${id}.png`);
  const args = [
    ...
    "--image", join(root, "referencias", "infographics", "contact-sheet.png")
  ];

  if (originals[id]) {
    args.push("--image", join(root, "referencias", "infographics", originals[id]));
  }

  args.push(
    "--prompt-file", join(root, "prompts", `slide-${id}.txt`),
    "--out", out,
    "--force"
  );
