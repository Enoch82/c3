...
const promptDir = join(root, "prompts-v2");
const renderDir = join(root, "renders-v2");

mkdirSync(promptDir, { recursive: true });
mkdirSync(renderDir, { recursive: true });

const globalStyle = `Style direction: match the original Nano Banana infographics in ../assets/infographics. Dense pedagogical cybersecurity infographic, not a presentation card UI. Use one dominant visual structure that fills 65-80% of the canvas: journey map, operating map, matrix, layered system, swimlane, funnel, threat map, or report board. Dark teal/black technical board, electric lime for priority and action, cyan for surfaces/system boundaries, orange for demo/risk/action, red only for negative impact. Use connected zones, arrows, grouped regions, small legends, labels, callouts, mini diagrams, and compact explanatory text. The result should teach by its spatial organization.`;
...
<head>
  ...
  <title>Estacion 10 - imagegen v2</title>
  <link rel="stylesheet" href="assets/imagegen.css">
</head>
<body>
<main class="deck rendered">
${slides.map((slide) => `<section class="render-slide" aria-label="Render v2 slide ${slide.id}"><img src="renders-v2/slide-${slide.id}.png" alt="Slide ${slide.id}: ${slide.title}"></section>`).join("\n")}
</main>
</body>
</html>
`;
...
writeFileSync(join(root, "estacion10-imagegen-v2-deck.html"), deck());
writeFileSync(join(root, "imagegen-prompts-v2.md"), `# Imagegen Prompts V2
...
References:
- Global style: \`../assets/infographics/contact-sheet.png\`
- Slide-specific originals when applicable: \`../assets/infographics/slide-2.png\`, \`slide-4.png\`, \`slide-5.png\`, \`slide-6.png\`, \`slide-7.png\`

${slides.map((slide) => `## Slide ${slide.id}: ${slide.title}

- Prompt: \`prompts-v2/slide-${slide.id}.txt\`
- Final render: \`renders-v2/slide-${slide.id}.png\`
- Closest original reference: ${slide.original ? `\`../assets/infographics/${slide.original}\`` : "global contact sheet only"}
`).join("\n")}
`);
...
console.log(`Wrote ${slides.length} v2 prompts.`);
